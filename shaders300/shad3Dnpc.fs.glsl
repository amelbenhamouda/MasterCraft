...
out vec3 fFragColor; // couleur qui sera affectée à chaque fragment
...
vec3 blinnPhong2() { // lumiere directionnelle  pour la "torche" du joueur
    ...
    float d = distance(vec3(0, 0, 0), vPosition_vs) * 0.1;
    return (uLightIntensity_lamp / (d * d) * (uKd2 * widotnormal + uKs2 * pow(halfdotnormal, uShininess_lamp)));
}

vec3 blinnPhong3() { // lumiere ponctuelle
    vec3 wo = normalize(-vPosition_vs); // half vector car la position caméra est l'origine du repère
...
void main() {
    ...
    if (uTorchEnable > 0.5) {
        ...
        float theta = dot(lightDir, vec3(-0.2, 0.2, 1));

        if (theta > cos(radians(12.5f))) { // remember that we're working with angles as cosines instead of degrees so a '>' is used.
            fFragColor = vec3(text3D) + blinnPhong3() + blinnPhong();
        }
        else {
            fFragColor = vec3(text3D) + blinnPhong();
        }
    }
    else {
        fFragColor = vec3(text3D) + blinnPhong();
